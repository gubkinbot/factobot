...
def echo_message(message):
    ...
    hidden_text = "Текст, который нужно скрыть"
    formatted_text = f"Это текст, в котором есть скрытый текст: \n\n* ||spoiler|| Скрытый текст: {hidden_text}*"
    bot.send_message(chat_id=message.chat.id, text=formatted_text, reply_markup=markup, parse_mode='Markdown2')
